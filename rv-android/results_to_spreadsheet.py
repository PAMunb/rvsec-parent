...
def get_errors_df(logcat_file_path: str, tasks: dict, exp01=False):
    ...
    rvsec_errors, _ = parse_logcat_file(logcat_file_path, exp01)
...
def get_coverage_df(logcat_file_path: str, tasks: dict, exp01=False):
    ...
    _, called_methods = parse_logcat_file(logcat_file_path, exp01)
    ...
    for clazz in called_methods:
        for method in called_methods[clazz][METHODS]:
            cov: RvCoverage = called_methods[clazz][METHODS][method]
...
@timer
def create_coverage():
    # header = ["apk", "rep", "timeout", "tool", "time", "class", "method", "params", "signature", "cov_act", "cov_method", "cov_rv_method"]
